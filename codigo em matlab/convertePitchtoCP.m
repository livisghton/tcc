...
dirOut = 'database/dataset/bd_CLP.csv';
dirOutFilter = 'database/dataset/bdFilter_CLP.csv';
bd = fopen(dirOut, 'w' );
...
for n=1:size(dirFileNames,1)
    ...
    %parameter.featureRate = sideinfo.f_chroma_norm.featureRate;
    ...
    for i = 1 : length(tempoFim)
        for j = k : length(f_chroma_norm)
            ...
            elseif (round(tempoInicio(i), 1) <= t && round(tempoFim(i), 1) > t && findChords(filterchords,chords(i))==0)
                ...
                chordFilter = f_chroma_norm(1:end, k).';
