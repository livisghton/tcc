...
dirOut = 'database/dataset/bd_CRP.csv';
dirOutFilter = 'database/dataset/bdFilter_CRP.csv';
bd = fopen(dirOut, 'w' );
...
for n=1:size(dirFileNames,1)
    ...
    [f_CRP,sideinfo] = pitch_to_CENS(f_pitch,parameter,sideinfo);
    %[f_CENS,sideinfo] = pitch_to_chroma(f_pitch,parameter,sideinfo);
    %[f_CENS,sideinfo] = pitch_to_CRP(f_pitch,parameter,sideinfo);
